...
export const BrowserMenu = ({
  ...
  const {
    loading, notes, tasks, blockedSites, stats, prefs,
    addNote, deleteNote, addTask, toggleTask, deleteTask,
    addBlockedSite, removeBlockedSite, startFocusSession,
    endFocusSession, updatePreference,
  } = useMenuData(isOpen);
  ...
  useEffect(() => {
    if (onFocusChange) {
      const remaining = focusMode && focusTimer ? formatTimer(focusTimer - focusElapsed) : "";
  ...
  const handleStartFocus = async (minutes: number) => {
    try {
      ...
      toast({ title: `Focus: ${minutes} minutos`, description: "Sitios distractores bloqueados" });
    } catch {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-800">
          ...
          {!loading && activeSection === "tools" && (
            <ToolsSection currentZoom={currentZoom} onZoomChange={onZoomChange} onClose={onClose} />
          )}
          {!loading && activeSection === "privacy" && (
            <PrivacySection prefs={prefs} stats={stats} onUpdatePref={handleUpdatePrivacyPref} />
          )}
          {!loading && activeSection === "workspace" && (
            <WorkspaceSection prefs={prefs} onThemeChange={handleThemeChange} onClose={onClose} />
          )}
          ...
          {!loading && activeSection === "notes" && (
            <NotesSection notes={notes} currentUrl={currentUrl} onAddNote={addNote} onDeleteNote={deleteNote} />
          )}
          {!loading && activeSection === "tasks" && (
            <TasksSection tasks={tasks} onAddTask={addTask} onToggleTask={toggleTask} onDeleteTask={deleteTask} />
          )}
          {!loading && activeSection === "media" && (
            <MediaSection onClose={onClose} />
          )}
          {!loading && activeSection === "devtools" && (
            <DevToolsSection currentUrl={currentUrl} onNavigate={onNavigate} onClose={onClose} onViewSource={onViewSource} />
          )}
