...
export const AddressBar = ({
	...
	useEffect(() => {
		...
		// Fetch inicial
		window.electron
			?.getPagePrivacyStats?.(domain)
			.then(setPageStats)
			.catch(() => {});

		// Poll cada 2s para actualizar el badge
		...
		// Escuchar bloqueos en tiempo real (actualización instantánea)
		window.electron?.onPrivacyBlocked?.((data) => {
		...
		return () => {
			...
			window.electron?.removePrivacyBlockedListener?.();
		};
	}, [domain, url]);
          ...
          `}>
					...
					<button
						...
						<div className="relative">
							...
							/>
							{/* Badge con número real de bloqueados - estilo Brave */}
							{pageStats.trackersBlocked > 0 && (
								<div className="absolute -top-2.5 -right-3 min-w-[16px] h-[16px] px-1 rounded-full bg-orange-500 flex items-center justify-center">
									<span className="text-[9px] font-bold text-white leading-none">
										{pageStats.trackersBlocked > 99
											? "99+"
									...
									<button
										...
										onClick={() =>
											toast({
												...
												description: "Se guardó en Descargas",
											})
										}
										className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl hover:bg-white/[0.04] transition-all duration-150 text-left group">
				...
				<div className="absolute top-full left-0 mt-2 z-50 animate-in">
					<div className="w-80 rounded-2xl bg-[#0d1117] border border-white/[0.08] shadow-2xl shadow-black/40 overflow-hidden">
						...
						<div className="p-4 space-y-3">
							...
							{/* Cookies bloqueadas - DATOS REALES */}
							<div className="flex items-center justify-between">
								<div className="flex items-center gap-2">
									<Globe className="w-3.5 h-3.5 text-slate-500" />
									<span className="text-xs text-slate-400">
										Cookies 3ros bloqueadas
									</span>
								</div>
								<span className="text-xs text-amber-400 font-bold">
									{pageStats.cookiesBlocked}
								</span>
							</div>

							{/* Datos ahorrados - DATOS REALES */}
							<div className="flex items-center justify-between">
								<div className="flex items-center gap-2">
								...
								<span className="text-xs text-emerald-400 font-bold">
									{pageStats.dataSavedBytes > 1048576
										? (pageStats.dataSavedBytes / 1048576).toFixed(1) + " MB"
										: (pageStats.dataSavedBytes / 1024).toFixed(0) + " KB"}
								</span>
							</div>
						</div>
							...
							<button
								onClick={() => {
									toast({
										...
										description: `${domain} fue bloqueado`,
									});
									setShowSecurityPanel(false);
								}}
